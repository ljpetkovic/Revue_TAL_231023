...
\textbf{Dans cette section, nous présentons les outils de la correction des sorties ROC utilisés dans le cadre de cette étude, puis une typologie des contaminations de corrections de ROC issues de l'analyse manuelle. Les analyses strictes des contaminations de ROC et de leurs corrections sont illustrées à travers des intersections, alors que nous employons des mesures de distance textuelle pour mener une évaluation plus souple des résultats de REN sur les sorties de ROC bruitées et leurs corrections automatiques. En dernier lieu, nous détaillons les calculs de la précision, du rappel et de F-score à l'aide de l'outil \textsc{Nerval}.}

\subsection{Outils de la correction des sorties ROC utilisés dans le cadre de cette étude}
\label{subsec:OUTILS-OCR-REN}
\textbf{Concernant la correction automatique des textes issus des transcriptions ROC, nous avons utilisé JamSpell, un outil développé en Python qui exploite le modèle de langue trigramme statistique\footnote{Pour plus de détails sur ce modèle \textit{cf}. \url{https://habr.com/en/articles/346618/}.} (grain mot), dont une partie des fonctionnalités est mise à disposition gratuitement sur le web.
...
De plus, si les modèles de langues pour le français et l'anglais, utilisés dans le cadre de cette étude, sont accessibles gratuitement, le modèle portugais est disponible uniquement dans la version payante de JamSpell (cette option n'a pas été testée).
Nous avons entraîné un modèle de langue pour JamSpell pour chacune des trois langues analysées dans notre étude. Nous avons sélectionné 40\% de chacun des corpus mis à disposition par ELTeC\footnote{soit 4 millions de tokens pour chaque modèle.} et nous en avons exclu les textes utilisés pour notre étude.}

Dans cette partie nous évaluons manuellement l'impact de deux types de modèles de langues pour la correction automatique utilisés avec l'outil JamSpell\footnote{Abbr.\ Jspll.}. Les premiers modèles sont ceux proposés par les créateurs de Jspll. et qui sont pré-entraînés pour l'anglais et le français. Le modèle pour le portugais n'est pas disponible dans la version gratuite du système. Les seconds types de modèles sont des modèles que nous avons entraînés nous même sur 40\% des textes de chacun des corpus ELTeC pour les trois langues (français, anglais et portugais) et nous évaluons ensuite ces modèles sur les jeux de test constitués tels que présentés en section \ref{sec:data}.
...
La colonne \textit{Brut} du tableau \ref{tab:ELTeC} montre qu'il y a globalement plus de types d'entités récupérées par spaCy sur Kraken (CER moyen :) que sur Tesseract (CER moyen :), ce qui vient étayer l'hypothèse que plus la qualité de la transcription est mauvaise plus les variations orthographiques sur les EN peuvent être nombreuses et plus il y a d'hapax dans les résultats de la REN. Nous venons à penser que si la correction automatique fonctionne bien le nombre des hapax sera réduit dans les sorties de REN.

En partant de ce postulat, nous observons que globalement les modèles Jspll pré-entraînés et entraînés sur ELTeC pour chaque langue permettent d'avoir des transcriptions d'une meilleure qualité que le texte brut car le nombre des types d'EN, donc des hapax, diminue. 

On note aussi que les modèles pré-entrainés de Jspll seraient globalement plus efficaces sur kraken que sur Tesseract.

Néanmoins, il apparaît que la correction automatique avec le modèle pré-entraîné de Jspll serait un peu moins efficace que celle du modèle entraîné sur le corpus ELTeC pour l'anglais. À l'inverse le modèle pré-entraîné de Jspll pour le français serait plus efficace que le modèle entraîné sur ELTeC pour le français, étant donné que le nombre d'EN augmente lors de la correction avec JamSpell-ELTeC pour le sous-corpus français océrisé avec Tesseract et kraken.
...
%------------------ déplacé dans la sous-section 6.1 le 21/02/24
