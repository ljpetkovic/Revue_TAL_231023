...
\subsection{Évaluation stricte des contaminations de ROC}
...
\textbf{Nous avons par ailleurs constaté qu'il existe deux types de silence dans la REN dans nos expériences : (i) Les EN qui ont été transcrites et non repérées par l'outil de REN et (ii) celles qui n'ont pas été transcrites dès l'origine. Le cas de Reynolds mets en exergue cette observation (tableau %\ref{tab:ELTeC_bon_mauvais})
 on a constaté que seuls 111 types d'EN ont été récupérés dans la configuration Kraken-\texttt{spaCy\_lg}.En effet, plus de 90\% des pages du PDF n'ont pas été OCRisées très probablement à cause du flou très visible sur les pages concernées. D'autres PDF ont connu le même sort dans de moindre proportions. De ce fait, une partie des entités manquantes dans les différentes configurations étudiées peuvent être dues non pas à la REN à proprement parler, mais à des transcriptions incomplètes. Nous n'avons pas mesuré l'impact de cette non transcription car il nous est apparu qu'elle était en faible proportion sur tout le corpus et le texte Reynolds était le seul cas très problématique. }
...
\begin{table}[h!]
    ...
    \caption{Nombre d'EN identifiées par \texttt{spaCy\_lg} dans les sous-corpus ELTeC  en fonction de différentes qualités de ROC déterminée par le CER calculé sur le modèle Tess. adapté à la langue du sous-corpus: bonne (fond gris foncé), moyenne (fond gris clair), mauvaise (fond blanc). 
    }
...
\begin{description}
  ...
  %\item[Vrais vrais négatifs (VVN)]: Aucune EN n'est détectée par le système dans aucune version du texte, car il n'y en a pas.
...
\subsection{Analyse manuelle des corrections sur des entités contaminées par la ROC}
