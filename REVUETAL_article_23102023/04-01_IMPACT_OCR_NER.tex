...
Grâce à un système de vote majoritaire, nous avons fusionné les annotations pour obtenir un \textit{gold standard} sur chaque version de chaque œuvre.
 ...
 En commençant par l'évaluation dite ``GLOBALE'' (tous les types d'entités), nous pouvons remarquer d'une part que les résultats obtenus sur les versions Tesseract sont meilleurs que ceux obtenus sur les versions Kraken. En outre, ce qui est plus étonnant, dans le cas de l'évaluation souple (où une partie de l'entité suffit à considérer la réponse bonne), ces résultats sont même meilleurs que ceux obtenus sur la version de référence.
