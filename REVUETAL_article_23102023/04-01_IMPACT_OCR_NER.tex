...
\textbf{Cette section décrit les outils de ROC et de REN, dont nous proposons des évaluations manuelle et automatique. Nous exposons des problématiques liées aux types d'évaluations stricte ou souple utilisés, ainsi qu'une typologie des contaminations de ROC pour une interprétation fine de l'évaluation stricte.}
...
Afin d'évaluer de manière plus supervisée l'influence du bruit ROC sur la REN, nous avons annoté un échantillon du corpus ELTeC français.
...
%L'accord inter-annotateur (Kappa de  Fleiss \cite{fleiss2013statistical}) était de $0.877$, significativement plus élevé sur la version de référence ($0.905$) que sur les versions ROC.
\textbf{L'accord inter-annotateur (Kappa de  Fleiss \cite{fleiss2013statistical}) était de $0.905$ sur la version de référence, ce qui est, significativement plus élevé que le score obtenu sur les versions ROC ($0.877$).}
 Nous avons pu observer que les désaccords étaient plus nombreux sur l'annotation des versions océrisées du fait des problèmes de tokenisation.
...
Grâce à un système de vote majoritaire, nous avons fusionné les annotations pour obtenir un \textit{gold standard} sur chaque version de chaque œuvre.
 Nous avons évalué \texttt{spaCy\_lg} sur cet échantillon, dont les résultats sont présentés dans le tableau \ref{tab:eval-supervise}. \textbf{À cet effet, nous proposons deux types d'évaluation que nous appelons ici \og{}stricte\fg{} et \og{}souple\fg{}. La première configuration ne prend en compte que les correspondances exactes des EN (p. ex. le mot contaminé \textit{Acques} ne sera pas considérée comme une EN, même si elle renvoie à une vraie EN \textit{Jacques}). En revanche, la configuration \og{}souple\fg{} considère une EN comme correcte, quelle que soit sa taille (une partie ou l'intégralité de l'EN, auquel cas le mot \textit{Acques} sera considéré comme une EN, bien qu'il s'agisse d'une forme contaminée de l'EN \textit{Jacques}).}
 
\textbf{Par rapport à} l'évaluation dite \textbf{\og{}globale\fg{} (qui concerne} tous les types d'\textbf{EN}), nous pouvons remarquer d'une part que les résultats obtenus sur les versions Tesseract sont meilleurs que ceux obtenus sur les versions Kraken \textbf{dans les configurations stricte et souple}.
...
%  comment expliquez-vous la meilleure performance sur les entités de lieux dans la configuration stricte ? [càd. pourquoi la F-mesure est meilleure sur les lieux que sur les autres EN avec l'évaluation stricte dans le tableau 8 ?]
  \textbf{Enfin, la meilleure performance sur les EN de lieux en termes de F-mesure dans la configuration stricte peut s'expliquer par le fait que ce type d'EN compte le plus souvent un token dans nos corpus (p. ex. \textit{Paris}), au contraire des noms de personnes notamment (p. ex. \textit{Daniel Eyssette}). À savoir, nous constatons une apparition très fréquente des EN \og{}globales\fg{} qui sont étalées sur plusieurs lignes, contrairement aux EN de lieux ; dans l'exemple \textit{Daniel Ey-sset-te}, la deuxième partie de l'EN est divisée en trois éléments\footnote{\textbf{séparés par des nouvelles lignes dans les fichiers d'origine, ici indiqués avec des tirets pour raison de lisibilité.}} qui représentent tous des parties d'une EN, mais qui ont très probablement posé problème à la REN et diminué les scores de la F-mesure. Dès lors, le rappel sur les noms de personnes descend beaucoup plus (nous obtenons plus de VP), mais les FP sont strictement inférieurs.}
\begin{table}[h!]
\centering
