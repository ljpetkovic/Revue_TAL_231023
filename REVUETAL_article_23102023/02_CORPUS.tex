...
\begin{table}[H]
    ...
    \caption{Statistiques sur le corpus TGB. (\texttt{spaCy\_lg} : modèle large de \texttt{spaCy}. Les deux dernières colonnes indiquent le nombre des EN).}
    \label{tab:TGBFra}
\end{table}
...
\begin{table}[h!]
    ...
    \caption{corpus français.\label{tab:ELTeCFra}}
    \end{subtable}

   \begin{subtable}[h]{1\textwidth}
    \small
    \input{TAB/corpus-ELTeCEng}
    \caption{corpus anglais.\label{tab:ELTeCEng}}
    \end{subtable}
    
    \begin{subtable}[h]{1\textwidth}
    \small
    \input{TAB/corpus-ELTeCPor}
    \caption{corpus portugais. \label{tab:ELTeCPor}}
    \end{subtable}
    \caption{Statistiques sur les corpus ELTeC.
